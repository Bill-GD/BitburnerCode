...
export async function main(ns) {
    ...
    if (ns.getHostname() == "home") ns.tprintf(" (!) grow1.js is running");
    ...
    while (true) {
        for (let i = 0; i < a; i++) {
            ...
            let available = ns.getServerMoneyAvailable(sv[i])
            let max = ns.getServerMaxMoney(sv[i])

            if (ns.hasRootAccess(sv[i]) && ns.getHackingLevel("home") >= ns.getServerRequiredHackingLevel(sv[i])) {
                if (sv[i] == "n00dles" && available <= 2e5) await ns.grow(sv[i]);
                if (sv[i] != "n00dles" && available <= 1e6) await ns.grow(sv[i]);
            }
        }
        await ns.sleep(1);
    }
