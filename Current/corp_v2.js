/** Version 2.1
 * Added Chemical as the 2nd division
 */
/** @param {NS} ns */
export async function main(ns) {
  ...
  const Enums = {
    BoostMaterial: {
      ...
      Chemical: [0, 100, 0, 30000],
      Restaurant: [0, 200, 150, 0],
      // Tobacco: [0, 0, 0, 0],
    },
    IndustryType: {
  ...
  if (getDivisions().includes(restaurantName)) stage[0] = 4;
  ...
  Object.values(Enums.IndustryType).forEach(industry => {
    const info = corp.getIndustryData(industry);
    ns.tprintf(`${industry}`);
    ns.tprintf(`${info.hardwareFactor}`);
    ns.tprintf(`${info.robotFactor}`);
    ns.tprintf(`${info.aiCoreFactor}`);
    ns.tprintf(`${info.realEstateFactor}`);
    ns.tprint(info.requiredMaterials);
    ns.tprintf(`${info.producedMaterials}`);
  });
    ...
    switch (stage[0]) {
      ...
      case 1: // more upgrades
        ...
        if (getFunds() > 140e9)
          stage = [2, 0];
        break;
      case 2: // expand to chemical
        try { corp.purchaseUnlock(Enums.Unlock.Export); } catch { }
        initDivision(Enums.IndustryType.Chemical, chemicalName);
        Object.values(ns.enums.CityName).forEach(city => {
          if (corp.getWarehouse(chemicalName, city).level < 2)
            corp.upgradeWarehouse(chemicalName, city, 2);
          corp.setSmartSupply(chemicalName, city, true);
          corp.exportMaterial(chemicalName, city, agricultureName, city, 'Plants', 'IPROD');
          corp.exportMaterial(agricultureName, city, chemicalName, city, 'Chemicals', 'IPROD');
        });
      ...
      case 3: // more upgrades
        ...
        if (corp.getDivision(agricultureName).numAdVerts < 20)
          try { corp.hireAdVert(agricultureName); } catch { }
